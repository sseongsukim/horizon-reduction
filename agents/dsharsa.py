...
class DSHARSAAgent(flax.struct.PyTreeNode):
    """Double SHARSA (DSHARSA) agent."""
    ...
    def low_value_loss(self, batch, grad_params):
        """Compute the low-level SARSA value loss."""
        q1, q2 = self.network.select('target_low_critic')(
            batch['observations'], goals=batch['low_value_goals'], actions=batch['actions']
        )
        if self.config['value_loss_type'] == 'bce':
            q1, q2 = jax.nn.sigmoid(q1), jax.nn.sigmoid(q2)
        ...
        v = self.network.select('low_value')(batch['observations'], batch['low_value_goals'], params=grad_params)
        if self.config['value_loss_type'] == 'bce':
            v_logit = v
            v = jax.nn.sigmoid(v_logit)
    ...
    def low_critic_loss(self, batch, grad_params):
        """Compute the low-level SARSA critic loss."""
        next_v = self.network.select('low_value')(batch['next_observations'], batch['low_value_goals'])
        if self.config['value_loss_type'] == 'bce':
            next_v = jax.nn.sigmoid(next_v)
        q = batch['low_value_rewards'] + self.config['low_discount'] * batch['low_value_masks'] * next_v

        q1, q2 = self.network.select('low_critic')(
            batch['observations'], batch['low_value_goals'], batch['actions'], params=grad_params
        )
        if self.config['value_loss_type'] == 'squared':
            critic_loss = ((q1 - q) ** 2 + (q2 - q) ** 2).mean()
        elif self.config['value_loss_type'] == 'bce':
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        rng, high_value_rng, high_critic_rng, low_value_rng, low_critic_rng, high_actor_rng, low_actor_rng = (
        ...
        loss = high_value_loss + high_critic_loss + low_value_loss + low_critic_loss + high_actor_loss + low_actor_loss
        return loss, info
    ...
    def sample_actions(
        ...
        n_observations = jnp.repeat(jnp.expand_dims(observations, 0), self.config['num_samples'], axis=0)
        n_subgoals = jnp.repeat(jnp.expand_dims(subgoals, 0), self.config['num_samples'], axis=0)
        n_orig_observations = jnp.repeat(jnp.expand_dims(orig_observations, 0), self.config['num_samples'], axis=0)

        for i in range(self.config['flow_steps']):
            t = jnp.full((self.config['num_samples'], *observations.shape[:-1], 1), i / self.config['flow_steps'])
            vels = self.network.select('low_actor_flow')(n_observations, n_subgoals, n_actions, t)
            n_actions = n_actions + vels / self.config['flow_steps']

        q = self.network.select('low_critic')(n_orig_observations, goals=n_subgoals, actions=n_actions).min(axis=0)
    ...
    def create(
        ...
        network_info = dict(
            ...
            low_critic=(low_critic_def, (ex_observations, ex_actions, ex_goals)),
            target_low_critic=(copy.deepcopy(low_critic_def), (ex_observations, ex_actions, ex_goals)),
