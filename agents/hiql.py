...
class HIQLAgent(flax.struct.PyTreeNode):
    ...
    def value_loss(self, batch, grad_params):
        ...
        (next_v1_t, next_v2_t) = self.network.select('target_value')(
            batch['high_value_next_observations'], batch['high_value_goals']
        )
        ...
        q1 = (
            batch['high_value_rewards']
            + (self.config['discount'] ** batch['high_value_subgoal_steps']) * batch['high_value_masks'] * next_v1_t
        )
        q2 = (
            batch['high_value_rewards']
            + (self.config['discount'] ** batch['high_value_subgoal_steps']) * batch['high_value_masks'] * next_v2_t
        )
    ...
    def high_actor_loss(self, batch, grad_params):
        ...
        nv1, nv2 = self.network.select('value')(batch['high_actor_next_observations'], batch['high_actor_goals'])
        v = (v1 + v2) / 2
        nv = (nv1 + nv2) / 2
        adv = nv - v
        ...
        if self.config['use_rep']:
            ...
            target = batch['high_actor_actions']
    ...
    def create(
        ...
        high_actor_def = GCActor(
            ...
            action_dim=config['rep_dim'] if config['use_rep'] else goal_dim,
            layer_norm=config['layer_norm'],
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            use_rep=True,  # Whether to use goal representation.
            rep_dim=128,  # Goal representation dimension.
            ...
            value_subgoal_steps=1,  # Value subgoal steps.
            ...
            actor_subgoal_steps=25,  # Actor subgoal steps.
